...
# noinspection PyPep8Naming,PyShadowingNames,DuplicatedCode
...
# noinspection PyMissingConstructor,DuplicatedCode,PyPep8Naming,PyShadowingNames,SpellCheckingInspection,PyUnresolvedReferences
class Particle(IBHP):
    # noinspection SpellCheckingInspection
    """
    ...
    def __init__(self, word_corpus: np.ndarray,
                 timestamp_array: np.ndarray, text_array: np.ndarray,
        ...
        :param sum_kernel_num: Number of base kernels
        ...
        self.word_num = len(self.word_corpus)
    ...
    def sample_first_particle_event_status(self, parameter_fixed: bool = False):
        ...
        self.K = 0
        self.w_0 = np.array([1 / self.sum_kernel_num] * self.sum_kernel_num)  # array, shape=(L, )
        self.v_0 = np.array([1 / self.word_num] * self.word_num)  # array, shape(S, )
    ...
    def log_hawkes_likelihood(self, n, lambda0, beta, tau):
        ...
        tau_expanded = np.expand_dims(tau, axis=0)
        ...
        if n == 1:
            ...
            for i in np.arange(2, n + 1):
                ...
                integral_delta_ti_1_tj = np.expand_dims(integral_delta_ti_1_tj, axis=1)
                exp_ti_1_tj = np.exp(- integral_delta_ti_1_tj / tau_expanded)
                integral_delta_ti_tj = self.timestamp_array[i - 1] - self.timestamp_array[: i - 1]
                integral_delta_ti_tj = np.expand_dims(integral_delta_ti_tj, axis=1)
                exp_ti_tj = np.exp(- integral_delta_ti_tj / tau_expanded)
        ...
        for i in np.arange(1, n + 1):
            ...
            prod_delta_ti_tj = np.expand_dims(prod_delta_ti_tj, axis=1)
            prod_exp_term = np.exp(- prod_delta_ti_tj / tau)
            ...
            c_i = np.argwhere(self.c[i - 1] != 0)[:, 0]  # nonzero element index for current event
            sum_j_prod = np.sum((prod_exp_term[:, c_i] / kappa_j_count_prod))
    ...
    def update_lambda0(self, n: int, n_mh: int, beta, tau, lambda0, parameter_a=3):
        ...
        lambda0_candidate_prior = sta.gamma.pdf(x=lambda0_candidate,
                                                a=parameter_a,
                                                scale=parameter_a / 2)  # probability of lambda0_candidate prior distribution
    ...
    def mh_update(self, n, n_iter=10000):
        ...
        updated_beta_list = [[] for i in np.arange(self.sum_kernel_num)]
        updated_tau_list = [[] for i in np.arange(self.sum_kernel_num)]
        ...
        for i in mh_iter:
            ...
            if i == 0:
                ...
                for idx in np.arange(self.sum_kernel_num):
    ...
    def update_hyperparameter(self, n, random_N: int = 1000, cartesian_N: int = 25, method: str = 'average'):
        ...
        """
        # draw candidate beta
        beta_candi_mat_random = sta.gamma.rvs(a=3, size=(random_N, self.sum_kernel_num))
        # calculate Cartesian product of beta arrays, the aim is to cover a more comprehensive sample space
        beta_candi_mat_cartesian = cartesian((sta.gamma.rvs(a=3, size=cartesian_N),) * self.sum_kernel_num)
        ...
        # draw candidate tau
        tau_candi_mat_random = sta.gamma.rvs(a=1.5, scale=0.15, size=(random_N, self.sum_kernel_num))
        # calculate Cartesian product of tau arrays
        tau_candi_mat_cartesian = cartesian((sta.gamma.rvs(a=1, size=cartesian_N),) * self.sum_kernel_num)  # (N, L)
        ...
        lambda0_candi_arr = sta.gamma.rvs(a=3, size=random_N + cartesian_N ** self.sum_kernel_num)
    ...
    def update_hyperparameter_grad_method(self, n, alpha_lambda, alpha_beta, alpha_tau, epsilon=0.001):
        ...
        while True:
            ...
            for l_index in np.arange(self.sum_kernel_num):
    ...
    def update_log_particle_weight(self, old_particle_weight, n: int):
        ...
        Tn_all = transfer_multi_dist_result_to_vec(self.text_array)  # shape=(S, )
        Tn = Tn_all[n - 1]
...
class StatusFixedParticle(Particle):
    ...
    def __init__(self, ibhp_instance, word_corpus, n_sample):
        ...
        """
        super(StatusFixedParticle, self).__init__(text_array=ibhp_instance.text,
                                                  timestamp_array=ibhp_instance.timestamp_array,
...
class ParameterFixedParticle(Particle):
    ...
    def __init__(self, ibhp_instance: IBHP, word_corpus):
        ...
        """
        super(ParameterFixedParticle, self).__init__(text_array=ibhp_instance.text,
                                                     timestamp_array=ibhp_instance.timestamp_array,
                                                     word_corpus=word_corpus,
                                                     fix_params=True,
...
class Filtering:
    ...
    def __init__(self, n_particle: int, n_sample: int, word_dict: np.ndarray, ibhp: IBHP):
        """
        Generates particles and initializes particle weights
        :param n_particle: number of particles
        :param n_sample: number of events
        """
        self.n_sample = n_sample
        # self.n_particle = n_particle + 1  # this num is used in fixed particle
        self.n_particle = n_particle
        self.particle_list = [Particle(word_corpus=word_dict,
                                       timestamp_array=ibhp.timestamp_array, text_array=ibhp.text,
                                       simulation_w=ibhp.w,
                                       simulation_v=ibhp.v,
                                       sum_kernel_num=ibhp.sum_kernel_num, fix_params=True) for
    ...
    def generate_first_event_status_for_each_particle(self, particle_idx_pair: Tuple[int, Particle]):
        ...
        if hasattr(particle, 'FLAG'):
            ...
            logging.info(f'[event 1, particle {particle_idx + 1}] Fixed Particle HP likelihood: {hp_lh}')
            logging.info(f'[event 1, particle {particle_idx + 1}] Updating Fixed particle weight')
            ...
            logging.info(f'[event 1, particle {particle_idx + 1}] Sampling Parameter Fixed particle status')
            particle.sample_first_particle_event_status(parameter_fixed=True)
            hp_lh = particle.log_hawkes_likelihood(n=1, lambda0=particle.lambda0, beta=particle.beta, tau=particle.tau)
            logging.info(f'[event 1, particle {particle_idx + 1}] Parameter Fixed Particle HP likelihood: {hp_lh}')
            logging.info(f'[event 1, particle {particle_idx + 1}] Updating Parameter Fixed particle weight')
            particle.update_log_particle_weight(old_particle_weight=self.particle_weight_arr[particle_idx], n=1)
            return particle_idx, particle
        else:
            logging.info(f'[event 1, particle {particle_idx + 1}] Sampling particle status')
            particle.sample_first_particle_event_status()
            # Update hyperparameters and triggering kernels
            logging.info(f'[event 1, particle {particle_idx + 1}] Updating hyperparameter')
            particle.update_hyperparameter(n=1)
            # Calculate and update the log particle weights
            hp_lh = particle.log_hawkes_likelihood(n=1, lambda0=particle.lambda0, beta=particle.beta, tau=particle.tau)
            logging.info(f'[event 1, particle {particle_idx + 1}] Particle HP likelihood: {hp_lh}')
            logging.info(f'[event 1, particle {particle_idx + 1}] Updating particle weight')
            particle.update_log_particle_weight(old_particle_weight=self.particle_weight_arr[particle_idx], n=1)
            return particle_idx, particle

    def generate_following_event_status_for_each_particle(self, n: int, particle_idx_pair: Tuple[int, Particle]):
        """
        ...
        if hasattr(particle, 'FLAG'):
            ...
            logging.info(f'[event {n}, particle {particle_idx + 1}] Fixed Particle HP likelihood: {hp_lh}')
            ...
            logging.info(f'[event {n}, particle {particle_idx + 1}] Sampling Parameter Fixed particle status')
            particle.sample_particle_following_event_status(n)
            hp_lh = particle.log_hawkes_likelihood(n=n, lambda0=particle.lambda0, beta=particle.beta, tau=particle.tau)
            logging.info(f'[event {n}, particle {particle_idx + 1}] Parameter Fixed Particle HP likelihood: {hp_lh}')
            logging.info(f'[event {n}, particle {particle_idx + 1}] Updating Parameter Fixed particle weight')
            particle.update_log_particle_weight(old_particle_weight=self.particle_weight_arr[particle_idx], n=n)
            return particle_idx, particle
        else:
            ...
            hp_lh = particle.log_hawkes_likelihood(n=n, lambda0=particle.lambda0, beta=particle.beta, tau=particle.tau)
            logging.info(f'[event {n}, particle {particle_idx + 1}] Particle HP likelihood: {hp_lh}')
            logging.info(f'[event {n}, particle {particle_idx + 1}] Updating particle weight')
            particle.update_log_particle_weight(old_particle_weight=self.particle_weight_arr[particle_idx], n=n)
            return particle_idx, particle
...
if __name__ == '__main__':
    ...
    pf = Filtering(n_particle=n_particle, n_sample=n_sample, ibhp=ibhp, word_dict=word_dict)
    ...
    for n in np.arange(2, n_sample + 1):
        ...
        particle_index_pair_list_after_update = []
        for pair in particle_index_pair_list_before_update:
            updated_pair = pf.generate_following_event_status_for_each_particle(n, pair)
            particle_index_pair_list_after_update.append(updated_pair)
