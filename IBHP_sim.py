...
class IBHP:
    ...
    def generate_first_event(self):
        ...
        logging.info(f'n=1时的factor kernel all：{self.kappa_all}')
        logging.info(f'n=1时的factor kernel：{self.kappa}')

        # sample v_k
        self.v = np.zeros((self.S, self.K))
        for k in np.arange(self.v.shape[1]):
            self.v[:, k] = np.random.dirichlet(self.v_0)
        logging.info(f'n=1时的词分布：{self.v}')
        ...
        logging.info(f'n=1时的时间戳：{self.timestamp}')

        # sample T_1
        multi_dist_prob = np.einsum('ij->i', self.v[:, np.argwhere(self.kappa != 0)[:, 0]]) / np.count_nonzero(
            self.kappa, axis=1)[0]
        logging.info(f'n=1时多项分布的参数p: {multi_dist_prob}')
    ...
    def generate_following_event(self, n: int):
        ...
        logging.info(f'n={n}时的factor kernel all：{self.kappa_all}')
        logging.info(f'n={n}时factor kernel的权重：{self.kappa}')
        ...
        logging.info(f'n={n}时多项分布的参数p: {multi_dist_prob}')
        logging.info(f'n={n}时多项分布的参数p求和: {np.sum(multi_dist_prob)}')
